  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full mb-2.5 rounded-xl overflow-hidden bg-surface-container-high shadow-lg">
        {coverUrl && <img src={coverUrl} alt={item.name ?? ''} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />}
        {!coverUrl && <TrackThumbnail size="xl" className="w-full h-full rounded-none" />}
        {onPlay && (
          <button
      ...
      <p className="text-body-sm font-semibold text-on-surface truncate leading-tight">{item.name}</p>
